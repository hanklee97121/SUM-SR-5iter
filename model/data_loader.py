...
class VideoData(Dataset):
    ...
    def __len__(self):
        self.len = len(self.splits[self.split_index][self.mode+'_keys'])
        return self.len
    ...
    def __getitem__(self, index):
        ...
        subsample_picks = np.array(self.video_data[video_name + '/picks'])
        sb = np.array(self.video_data[video_name + '/change_points'])
        n_frames = np.array(self.video_data[video_name + '/n_frames'])
        if self.mode == 'test':
            return frame_features, video_name, subsample_picks, sb, n_frames
        else:
